let summaryMixin = {
  computed: {
    ...
    downHoleSummaryList: function () {
      const showRoutesKey = this.$store.getters["getShowRoutes"]
        .map((item) => pageTypeMap[item.type])
        .filter(Boolean);
    ...
    summaryList: function () {
      ...
      if (curPageType === PageTypeEnum.MONITOR) {
        ...
        Object.keys(countTypeMap).forEach((filterKey) => {
          ...
          const [normal_count, warn_count, error_count] = countTypeMap[
            filterKey
          ].map((key) => Number(monitorEventNum[key]));
          const item = {
            title: filterKey,
      ...
      const [normal_count, warn_count, error_count] = summaryValueArr.map(
        (key) => Number(monitorEventNum[key])
      );

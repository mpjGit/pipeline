        ...
        <div class="form-wrap row">
          <div class="col-span-7">
            ...
            <div class="grid-left">
              ...
              <ValueGroup v-if="localRecordDetail.status === 1" name="故障内容" :value="FAULT_MAP[localRecordDetail.fault]" />
              <ValueGroup v-if="localRecordDetail.status === 2" name="报警内容" :value="FAULT_MAP[localRecordDetail.fault]" />
             <!-- <ValueGroup name="进气口压强" :value="localRecordDetail.entrance_pressure + ' Kpa'" />-->
            </div>
...
import {isWarnMsg, powerToString, timeToString} from "@/utils/tool";
...
export default {
  ...
  methods: {
    processDelete() {
      this.showModal({
        ...
        onConfirm: async () => {
          ...
          await this.$store.dispatch('notification/deleteHistory', {ids, type: PageTypeEnum.DOWNHOLE});
    ...
    ...mapActions({
      'showModal': 'showModal',
      'hideModal': 'hideModal',
      'fetchPage': 'downholeHistoryList/fetchPage',
      'fetchRecord': 'downholeHistoryList/fetchRecord'
    })
  },
  computed: {
    formatRows: function () {
      return this.$store.state.downholeHistoryList.list.map(value => {
    ...
    pagination: function () {
      return {
        total: this.$store.state.downholeHistoryList.total,
        pageSize: this.$store.state.downholeHistoryList.pageSize,
        currentPage: this.$store.state.downholeHistoryList.currentPage,
      }
    }

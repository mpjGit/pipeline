...
import {isWarnMsg, isFaultMsg} from "@/utils/tool";
...
export default {
  ...
  computed: {
    ...
    // 设备列表
    rows: function () {
      return this.$store.state.downholeList.list.map(item => {
        let status = NORMAL;
    ...
    configDetail: function () {
      return this.$store.state.downholeDeviceConfig.configDetail;
    },

    // 已选中的所有设备信息
    configDetailList: function () {
      return this.$store.state.downholeDeviceConfig.configList;
    },
    ...
    pagination: function () {
      return {
        total: this.$store.state.downholeList.total,
        pageSize: this.$store.state.downholeList.pageSize,
        currentPage: this.$store.state.downholeList.currentPage,
      }
    },

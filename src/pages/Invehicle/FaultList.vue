...
export default {
  ...
  methods: {
    ...mapActions({
      'showModal': 'showModal',
      'hideModal': 'hideModal',
      'fetchPage': 'inVehicleFaultList/fetchPage',
      'fetchRecord': 'inVehicleFaultList/fetchRecord',
      'processRecord': 'inVehicleFaultList/processRecord'
    }),
    ...
    processExport: function () {
      this.showModal({
        ...
        onConfirm: () => {
          // 执行导出
          request.post('exwfault', {
            username: this.username,
            id: this.currentSelected.map(o => o.id)
          }).then(res => {
            const {code} = res;
            if(code === 0) {
              request.download(`download?num=${DOWNLOAD_TYPE.downhole_fault}`);
            }
          })
          this.hideModal();
        },
  ...
  computed: {
    formatRows: function () {
      return this.$store.state.inVehicleFaultList.list.map(value => {
        return {
          ...value,
    ...
    pagination: function () {
      return {
        total: this.$store.state.inVehicleFaultList.total,
        pageSize: this.$store.state.inVehicleFaultList.pageSize,
        currentPage: this.$store.state.inVehicleFaultList.currentPage,
      }
    },

...
const alertList = {
    ...
    actions: {
        fetchPage: function(context, {page , query}) {
            request.post(`getWarnMileage?${query}`, {
        ...
        fetchRecord: function(context, id) {
            request.post('mileageWarnByUsername', {
                username: context.rootState.user.username,
                id
            }).then(async (res) => {
                const item = res[0];
                const data = {
                    ...
                    manholeFlag: Number(1),
                    liqFlag: Number(1),
                    temperatureFlag: Number(1),
                    concentrationFlag: Number(1),
                };
        ...
        processRecord: function (context, {id, implementer}) {
            ...
            request.patch('handleMileageWarn', {
                username: context.rootState.user.username,
                id,
                ...
                context.dispatch('notification/refreshMileageWarnList', {}, {root: true});
